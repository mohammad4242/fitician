...
export function AdminBillingOrderDetailPage() {
  ...
  if (state === "error" || order === null) return <p className="access-admin-status access-admin-status--error" role="alert">{t("adminAccess.orderNotFound", "سفارش پیدا نشد.")}</p>;

  return (
    <main className="access-admin-page fitsho-page">
      <div className="access-admin-page__container">
        <Link className="access-admin-back-link" to="/admin/billing/orders">{t("adminAccess.backToOrders", "بازگشت به سفارش‌ها")}</Link>
        <header className="access-user-detail__header">
          ...
          {order.user_id !== null && <Link className="access-admin-button access-admin-button--quiet" to={`/admin/billing/users/${order.user_id}`}>{t("adminAccess.viewUserAccess", "مشاهده دسترسی کاربر")}</Link>}
        </header>

        <section className="access-order-detail-card" aria-labelledby="admin-order-snapshot-title">
          <header><div><p className="eyebrow eyebrow--accent">{t("adminAccess.order")}</p><h2 id="admin-order-snapshot-title">{t("adminAccess.orderSnapshot", "خلاصه سفارش")}</h2></div><span className="access-order-card__status"><b>{order.status}</b><small>{order.provider}</small></span></header>
          <dl>
            ...
            <div><dt>{t("adminAccess.trainingTerm")}</dt><dd>{order.duration_weeks_snapshot} {t("adminAccess.weeks", "هفته")}</dd></div>
            ...
            <div><dt>{t("adminAccess.paidAt", "پرداخت‌شده در")}</dt><dd>{order.paid_at === null ? "—" : formatDate(order.paid_at, english)}</dd></div>
            <div><dt>{t("adminAccess.refundedAt", "Refund شده در")}</dt><dd>{order.refunded_at === null ? "—" : formatDate(order.refunded_at, english)}</dd></div>
        ...
        <section aria-label={t("adminAccess.transactions")} className="access-transaction-section">
          ...
          {order.transactions.length === 0 && <p className="access-admin-status">{t("adminAccess.noTransactions", "تراکنشی ثبت نشده است.")}</p>}
          {order.transactions.length > 0 && <div className="access-transaction-list">{order.transactions.map((transaction) => <article className="access-transaction-card" key={transaction.id}><header><strong>{transaction.id}</strong><span>{transaction.status}</span></header><dl><div><dt>{t("adminAccess.provider")}</dt><dd>{transaction.provider}</dd></div><div><dt>{t("adminAccess.providerReference")}</dt><dd>{transaction.provider_reference ?? "—"}</dd></div><div><dt>{t("adminAccess.amount")}</dt><dd>{formatAmount(transaction.amount_irr, transaction.currency, english)}</dd></div><div><dt>{t("adminAccess.verifiedAt", "تأییدشده در")}</dt><dd>{transaction.verified_at === null ? "—" : formatDate(transaction.verified_at, english)}</dd></div><div><dt>{t("adminAccess.failedAt", "ناموفق در")}</dt><dd>{transaction.failed_at === null ? "—" : formatDate(transaction.failed_at, english)}</dd></div><div><dt>{t("adminAccess.refundedAt", "Refund شده در")}</dt><dd>{transaction.refunded_at === null ? "—" : formatDate(transaction.refunded_at, english)}</dd></div></dl></article>)}</div>}
        </section>
      </div>
    </main>
  );
