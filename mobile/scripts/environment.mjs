...
const productionApiBaseUrl = endpointConfig.apiBaseUrl;
const productionFrontendOrigin = endpointConfig.frontendOrigin;
const productionAppLinkHost = new URL(productionFrontendOrigin).hostname;
...
export function validateEnvironment(expectedVariant, values, options = {}) {
  ...
  if (expectedVariant === "production" && apiUrl.replace(/\/+$/u, "") !== productionApiBaseUrl) {
    throw new Error(
      `EXPO_PUBLIC_API_BASE_URL must equal the approved production origin ${productionApiBaseUrl}`,
    );
  }
  if (
  ...
  if (
    ...
    throw new Error(
      `EXPO_PUBLIC_FRONTEND_ORIGIN must equal the approved production origin ${productionFrontendOrigin}`,
    );
  }
  ...
  if (expectedVariant === "production" && appLinkHost !== productionAppLinkHost) {
    throw new Error(`FITICIAN_APP_LINK_HOST must be ${productionAppLinkHost} for production builds`);
  }
}
