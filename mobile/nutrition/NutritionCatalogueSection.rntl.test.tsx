...
test("admin sees safe error diagnostics for a catalogue request", async () => {
  isAdmin = true;
  mockUseMobileAuth.mockReturnValue({ request: jest.fn(), upload: jest.fn(), user: { is_admin: true } } as never);
  mockUseQuery.mockImplementation(({ queryKey }) => {
    const key = queryKey as readonly unknown[];
    ...
    return {
      data: undefined,
      error: new ApiError(503, "catalogue provider secret", null, "SERVICE_UNAVAILABLE", {
        meta: { current_state: "catalogue" },
        requestId: "catalogue-admin-request-1",
      }),
      isError: true,
      isFetching: false,
      isPending: false,
  ...
  expect(diagnostics).toHaveTextContent(/شناسه پیگیری: catalogue-admin-request-1/u);
