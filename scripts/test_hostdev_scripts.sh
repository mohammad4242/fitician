...
grep -Fq '127.0.0.1:5433/fitician' "$launcher"
grep -Fq -- '--port 8002' "$launcher"
grep -Fq 'COOKIE_SECURE="false"' "$launcher"
grep -Fq 'SESSION_COOKIE_NAME="fitician_session"' "$launcher"
grep -Fq ':5174' "$launcher"
grep -Fq 'FITICIAN_HOSTDEV_LAN_IPS' "$launcher"
grep -Fq 'for hostdev_lan_ip in $hostdev_lan_ips' "$launcher"

grep -Fq 'destination_project="fitician-hostdev"' "$clone_script"
